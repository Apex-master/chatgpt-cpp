...
void TextEditor::InsertText(const char* aValue) // updated to insert at bottom
{
	if (aValue == nullptr)
		return;

	auto pos = Coordinates(static_cast<int>(mLines.size()) - 1, mLines.back().size());
	auto start = std::min(pos, mState.mSelectionStart);
	int totalLines = InsertTextAt(pos, aValue);
